...
A module is essentially a folder containing Terraform configuration files (after `terraform init` you'll find the files under `.terraform/modules/polyai_service_vpc`).
...
Every entry in the `module.polyai_service_vpc` block (e.g. `name`, `cidr`, `azs` etc...) is actually **defined as a variable** in the module directory (can be found under `.terraform/modules/polyai_service_vpc/variables.tf`).
...
The next obvious step is to migrate our `aws_instance.polyai_dev` instance within the created VPC. 

To do so, add the following property to the `aws_instance.polyai_dev` resource:

```diff
+ subnet_id   = module.polyai_service_vpc.public_subnets[0]
```

As can be seen, we used `module.polyai_service_vpc.vpc_id` is a reference the VPC id created by our module. 

The `module.polyai_service_vpc.vpc_id` attribute is known as a module **output**. 
...
In you take a closed look in `.terraform/modules/polyai_service_vpc/outputs.tf`, you'll see how outputs are defined:
...
To create your security group within your VPC, add the following property to the `aws_security_group.polyai_dev_sg` resource:
...
Change the following attribute in `polyai_service_vpc` module:
...
The `aws_instance.polyai_dev` configuration also uses a hard-coded AMI ID, which is only valid for the specific region. 
...
### :pencil2: (Optional) Build cloud agnostic app 
