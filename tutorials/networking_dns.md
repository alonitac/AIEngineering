...
1. First, get the list of the root-level DNS servers:
```shell
dig . NS
```

2. Pick one of the root-level domain names. We will query this server to get the hostname of the *com* top-level domain:
```shell
...
> [!NOTE]
> We've registered a domain that will be shared by all of the group: `fursa.click`. You can use any sub-domain of `fursa.click` for your services, e.g. `my-name-dev.fursa.click`. 
...
2. Choose the hosted zone associated with our domain `fursa.click`.

3. Choose **Create record**\.

4. Define an A record for a custom sub-domain of yours (e.g. `my-name.fursa.click`), the record value is an IP address of your EC2 instance created in a previous tutorial. 
...
## Update the CD pipeline to use domain names

In [T002](../tasks/t002_yolo_cicd_workflow.md), the CD pipeline connects to instances using raw Elastic IPs.

Update the pipeline to connect via `dev.<your-name>.fursa.click` and `prod.<your-name>.fursa.click` instead.

> [!NOTE]
> You must follow the feature-branch development lifecycle practiced: create a feature branch from `main`, test the change via `dev`, then open a PR into `main`.
